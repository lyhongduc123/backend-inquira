...
class PaperDetailChatService:
    ...
    async def stream_chat(
        ...
        messages = await conversation_service.repo.get_messages_by_conversation(conversation_id)
...
"""
        ...
        async for chunk_text in self.llm_service.stream_citation_based_response(
            ...
            token = chunk_text.get("text", "") if isinstance(chunk_text, dict) else str(chunk_text)
            full_response += token
            async for evt in stream_event(name="chunk", data={"text": token}):
                yield evt
