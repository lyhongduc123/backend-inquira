...
class PaperRetrievalService:
    ...
    Uses dependency injection for better testability and reduced coupling.
    """
    ...
    def __init__(
        ...
        """
        Initialize PaperRetrievalService with dependency injection.
    ...
    async def search(
        ...
        papers = transformer.batch_normalized_to_papers(results)
        return papers
    ...
    async def hybrid_search(
        ...
        papers = transformer.batch_normalized_to_papers(enriched_papers[:final_limit])
    ...
    async def _enrich_with_openalex(
        self, normalized_semantic_results: List[NormalizedResult]
        ...
        """
        openalex_provider = self.providers[RetrievalServiceType.OPENALEX]
        if not isinstance(openalex_provider, OpenAlexProvider):
            logger.error(
                "[HybridSearch] OpenAlex provider not properly configured"
            )
            return normalized_semantic_results
        ...
        for doi, normalized_semantic_result in doi_to_semantic.items():
            ...
            if openalex_result:
                normalized_openalex_result = openalex_provider.normalize_result(openalex_result)
    ...
    def _merge_semantic_and_openalex(
        self, semantic_result: NormalizedResult, openalex_result: NormalizedResult
    ) -> NormalizedResult:
        """
        ...
        merged_dict["citation_count"] = max(
            semantic_result.citation_count or 0,
            openalex_result.citation_count or 0,
        )
        ...
        # Topics, keywords, concepts (OpenAlex has better taxonomy)
        ...
        merged_dict["corresponding_author_ids"] = oa_dict.get("corresponding_author_ids", [])
        merged_dict["authorships"] = oa_dict.get("authorships", [])
        merged_dict["institutions_distinct_count"] = oa_dict.get("institutions_distinct_count", 0)
        merged_dict["countries_distinct_count"] = oa_dict.get("countries_distinct_count", 0)
        
        merged_dict["semantic_authors"] = [author.model_dump() for author in semantic_result.authors]
        
        for i, author in enumerate(merged_dict["semantic_authors"]):
            if author.get("author_id") and not author.get("openalex_author_id"):
                author["openalex_author_id"] = author["author_id"]
            if i < len(merged_dict.get("authors", [])):
                merged_dict["authors"][i].update(author)
            else:
                merged_dict["authors"].append(author)
        ...
        if openalex_result.paper_id:
            merged_dict["external_ids"]["OpenAlex"] = openalex_result.paper_id.removeprefix("https://openalex.org/")

        merged_dict["openalex_data"] = oa_dict.get("raw_data")
        merged_dict["semantic_data"] = semantic_result.model_dump().get("raw_data")


        return NormalizedResult(**merged_dict)
    ...
    async def get_paper_if_exists(
        ...
        """
        paper = await self.paper_service.get_paper_if_exists(external_ids, source)
        return paper

    async def get_tei_xml(self, paper: Paper) -> Optional[str]:
        """
        ...
        Args:
            paper: Paper object with openalex_id
    ...
    async def get_pdf_paper(self, paper: Paper) -> Optional[bytes]:
        """
        ...
        try:
            ...
            if result.get("is_open_access"):
                ...
                if fallback_pdf_url and fallback_pdf_url != paper.pdf_url:
                    logger.info(
                        f"Trying fallback PDF URL from access_info: {fallback_pdf_url}"
                    )
                    pdfBytes = await self.paper_retriever.download_pdf(
                        str(fallback_pdf_url)
                    )
                    if pdfBytes:
                        return pdfBytes
                    else:
                        logger.warning(
                            f"Failed to download from fallback URL: {fallback_pdf_url}"
                        )
